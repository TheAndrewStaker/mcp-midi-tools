...
server.registerTool('hydra_set_engine_params', {
  description: HYDRA_DEV_MODE_PREAMBLE + [
    ...
    'reorder for you.',
    '',
    'DIAGNOSTIC USE — when the user reports an unwanted artifact in a',
    'recently-built patch (stray ringing, pitch creep, wash on the tail),',
    'do NOT batch unrelated fixes here. Bypass one suspect FX block at a',
    'time via `set_bypass` and re-audition with `play_note` / `play_chord`',
    'before changing values. Batching is right for confident builds;',
    'isolation is right for chasing artifacts.',
    '',
