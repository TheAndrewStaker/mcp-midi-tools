...
export function registerPresetTools(server: McpServer): void {
  server.registerTool('apply_preset', {
    description: [
      ...
      'knobs are exposed. AM4 example: `reverb.type = "Hall, Large"` is a',
      'fixed-decay algorithm and silently drops writes to `reverb.time`;',
      '`reverb.type = "Hall, Large Deep"` exposes `time`. Same pattern on',
      'amp.type (e.g. real-circuit Vox AC30 lineages have no `master` knob).',
      ...
      'Skipping this step is the #1 cause of the "dropped X param" warning',
      'in the apply_preset response.',
