...
export function registerAxeFxIIMetaTools(server: McpServer): void {
  ...
  server.registerTool('axefx2_probe_sysex', {
    description: [
      'Send a raw SysEx byte sequence to the Axe-Fx II AND capture any',
      'inbound MIDI traffic the device emits in response. The diagnostic',
      'analog of `send_sysex`, but with the wire visibility that decode',
      'work requires — `send_sysex` is fire-and-forget and leaves you',
      'half-blind for probe-and-observe.',
      '',
      'Use this tool for protocol RE — e.g. probing undecoded function',
      'bytes, sweeping payload positions, or verifying that a write',
      'actually ACKs vs silently dropping. Not for production preset',
      'edits: use axefx2_set_param / apply_preset for those.',
      '',
      ...
      'responsibility — most Fractal functions use XOR-of-everything-up-',
      'to-cs, AND 0x7F (see SYSEX-MAP-AXE-FX-II.md for the exact rule).',
      '',
      ...
      '`capture_ms` and retry), or (c) the input port isn\'t open (check',
      'list_midi_ports — Axe-Fx II should appear as bidirectional).',
      '',
      'NOTE: this tool DOES affect the buffer-dirty flag. If the bytes',
      'you send happen to be a write-class function (0x02 SET_PARAM, 0x05',
      'SET_GRID_CELL, etc.), the device emits a state-broadcast triple',
      'which marks the buffer dirty per the device-sourced classifier',
      'wired in midi.ts. That\'s correct behavior — your probe DID edit',
      'the buffer. Use `axefx2_switch_preset({ slot: 1 })` to reload from',
      'flash and reset to a clean baseline before the next probe.',
    ].join('\n'),
    inputSchema: {
