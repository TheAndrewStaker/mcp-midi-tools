...
Fractal's factory presets are stored in the AM4's flash and re-loadable
without a backup. The `restore_defaults` tool (or `am4_restore_factory`)
puts any single slot or range back to factory bytes.

```
You: Restore A01 to factory.
AI:  [calls restore_defaults(port='am4', from='A01')]
     A01 restored: "Big Plexi" (factory preset 14).
```
