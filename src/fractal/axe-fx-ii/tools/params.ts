...
export function registerAxeFxIIParamTools(server: McpServer): void {
  ...
  server.registerTool('axefx2_set_param', {
    description: [
      ...
      '',
      'VOLUME LANGUAGE: when the user says "louder / more drive / more',
      'reverb", pick the right knob — `amp.input_drive` (aliased `gain`)',
      'is INPUT DRIVE (changes character), `amp.master_volume` (aliased',
      '`master`) is amp master (preserves character), `amp.level` is the',
      'amp\'s output knob, `cab.level` is post-cab dB trim (-80..+20) for',
      'preset matching, and `<fx>.mix` (0..100%) is each effect\'s wet/dry.',
      'Full cross-device cheat sheet: docs/VOLUME-CONTROL.md.',
      '',
