...
import {
  KNOWN_PARAMS,
  type AxeFxIIParam,
} from '@/fractal/axe-fx-ii/params.js';
import { AXE_FX_II_BLOCKS, type AxeFxIIBlock } from '@/fractal/axe-fx-ii/blockTypes.js';
import { displayToWire, wireToDisplay } from '@/fractal/axe-fx-ii/setParam.js';
import { PARAM_ALIASES_AXEFX2 } from '@/fractal/axe-fx-ii/paramAliases.js';
...
export function makeEncode(param: AxeFxIIParam): ParamSchema['encode'] {
  return (value: number | string): number => {
    ...
    if (hasCalibration) {
      const min = param.displayMin as number;
      const max = param.displayMax as number;
      if (num < min || num > max) {
...
export function buildBlocks(): Record<string, BlockSchema> {
  ...
  for (const key of Object.keys(KNOWN_PARAMS)) {
    ...
    blocks[block].params[name] = {
      ...
      unit: unitFor(param),
      display_min: param.controlType === 'select' || param.controlType === 'switch' ? undefined : param.displayMin,
      display_max: param.controlType === 'select' || param.controlType === 'switch' ? undefined : param.displayMax,
