...
async function main(): Promise<void> {
  ...
  let pass = true;
  try {
    // Scenario: hydra_apply_patch with save: true but no save_authorized → refuse.
    ...
    console.log(refused ? '✓ hydra_apply_patch refuses save:true without save_authorized' : '✗ FAIL — gate did not refuse');
    if (!refused) {
      console.log('  Response:\n' + text.split('\n').slice(0, 6).map((l) => `    ${l}`).join('\n'));
      pass = false;
    }

    // Scenario: hydra_apply_patch without save → no save-auth refusal.
